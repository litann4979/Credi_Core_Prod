...
/* Stretch content in fullscreen/TV mode to avoid empty bottom space */
body.dashboard-fullscreen .tv-frame {
    display: flex;
    flex-direction: column;
}

body.dashboard-fullscreen .dashboard-main-grid {
    flex: 1 1 auto;
    min-height: 0;
    margin-bottom: 8px !important;
}

body.dashboard-fullscreen .dashboard-footer {
    margin-top: auto;
    padding-bottom: 2px;
}

body.dashboard-fullscreen .right-panel-stack {
    display: flex;
    flex-direction: column;
    height: 100%;
    padding-top: 4px;
...
body.dashboard-fullscreen .right-panel-performance {
    flex: 1 1 auto;
    min-height: 0;
}

body.dashboard-fullscreen .metric-stack {
    flex: 1 1 auto;
    display: flex;
    flex-direction: column;
...
/* Fullscreen / TV: larger employee table data (overrides Tailwind text-* on cells) */
body.dashboard-fullscreen .live-dashboard-employee-table thead th {
...
body.dashboard-fullscreen .live-dashboard-employee-table .table-row td {
...
body.dashboard-fullscreen .live-dashboard-employee-table .table-row td .text-sm {
    font-size: 1.2rem !important;
    line-height: 1.35 !important;
}

body.dashboard-fullscreen .live-dashboard-employee-table .table-row td .text-lg {
    font-size: 1.35rem !important;
}

body.dashboard-fullscreen .live-dashboard-employee-table .rank-medal {
    font-size: 1.15rem !important;
}

body.dashboard-fullscreen .live-dashboard-employee-table .avatar-img {
    width: 40px !important;
    height: 40px !important;
}

body.dashboard-fullscreen .live-dashboard-employee-table .badge-excellent,
body.dashboard-fullscreen .live-dashboard-employee-table .badge-good,
body.dashboard-fullscreen .live-dashboard-employee-table .badge-average,
body.dashboard-fullscreen .live-dashboard-employee-table .badge-low {
    font-size: 1rem !important;
    padding: 0.4rem 0.95rem !important;
}

body.dashboard-fullscreen #table-page-indicator {
...
<body class="overflow-x-hidden overflow-y-auto bg-[#020617]">
    @php
        ...
        // Hide Daily Score only for month view; keep visible for single-day date filter.
    ...
    <div class="w-screen min-h-screen tv-frame">
        ...
        <div class="glass-card px-4 py-2 mb-2 flex flex-wrap justify-between items-center">
            ...
            <div class="mt-3 md:mt-0 md:text-right w-full md:w-auto">
                ...
                <div class="mt-2 flex flex-wrap items-center justify-start md:justify-end gap-2">
                    ...
                    <button
                        type="button"
                        class="bg-[#0b2f58] border border-[#2c4c7a] text-cyan-100 text-xs md:text-sm rounded-lg px-2.5 py-1.5 hover:bg-[#184374] transition-colors"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-3 mb-3 items-stretch dashboard-main-grid">
            {{-- LEFT: Employee Table --}}
            <div class="lg:col-span-8 h-full">
                <div class="glass-card theme-table overflow-hidden h-full flex flex-col">
                    <div class="overflow-hidden flex-1">
                        <table class="live-dashboard-employee-table w-full min-w-[760px] table-auto">
                        <thead>
                            <tr class="border-b border-[#2a4b7a] bg-[#0a1733]">
                                ...
                                <th class="px-4 py-2 text-center text-blue-200 font-bold text-xs uppercase tracking-wider {{ $hideDailyScore ? 'hidden' : '' }}" id="daily-score-head">Daily Score</th>
                        ...
                        <tbody id="employee-table-body">
                            ...
                            <tr class="table-row employee-row">
                                ...
                                <td class="px-4 py-2 text-center {{ $hideDailyScore ? 'hidden' : '' }}" data-daily-score-cell>
                                ...
                                <td class="px-4 py-2 text-center">
                                    <span class="inline-block px-2 py-0.5 rounded-lg bg-[#1e3a5f] text-white font-bold text-sm">{{ $emp['monthly_score_display'] ?? ($emp['score_display'] ?? number_format((float) ($emp['score'] ?? 0), 1)) }}</span>
                                </td>
                                <td class="px-4 py-2 text-center">
                                    @if(($emp['status'] ?? '') == 'Excellent')
                                        <span class="badge-excellent px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">✅ Excellent</span>
                                    @elseif(($emp['status'] ?? '') == 'Good')
                                        <span class="badge-good px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">✅ Good</span>
                                    @elseif(($emp['status'] ?? '') == 'Average')
                                        <span class="badge-average px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">✅ Average</span>
                                    @else
                                        <span class="badge-low px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">✅ Low</span>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="7" class="px-6 py-10 text-center text-gray-300 font-semibold">No employee performance data available for selected filter.</td>
                            </tr>
                            @endforelse
                        </tbody>
                        </table>
            ...
            <div class="lg:col-span-4 space-y-4 h-full right-panel-stack">
                <div class="glass-card p-4 right-panel-leave">
                    ...
                    <div class="space-y-6 metric-stack">
                        <div>
                            ...
                            <div class="w-full bg-gray-700 rounded-full h-3 overflow-hidden">
                                <div class="bg-gradient-to-r from-blue-500 to-indigo-600 h-3 rounded-full progress-bar" id="metric-target-bar" style="width: {{ min(100, (float) ($performanceMetrics['target_achievement_percent'] ?? 0)) }}%"></div>
                            </div>
                        </div>
                        <div>
                            <div class="flex justify-between mb-2">
                            ...
                            <div class="w-full bg-gray-700 rounded-full h-3 overflow-hidden">
                                <div class="bg-gradient-to-r from-emerald-500 to-green-600 h-3 rounded-full progress-bar" id="metric-lead-bar" style="width: {{ min(100, (float) ($performanceMetrics['lead_entry_percent'] ?? 0)) }}%"></div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="w-full bg-gray-700 rounded-full h-3 overflow-hidden">
                                <div class="bg-gradient-to-r from-amber-500 to-orange-600 h-3 rounded-full progress-bar" id="metric-discipline-bar" style="width: {{ min(100, (float) ($performanceMetrics['discipline_percent'] ?? 0)) }}%"></div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        {{-- Footer --}}
        <div class="text-center mt-2 mb-0 text-gray-400 text-sm tracking-wide dashboard-footer">
    ...
    <script>
        ...
        const leaderPhoto = @json($leaderPhoto);
        const hideDailyScore = @json($hideDailyScore);
        const dashboardDataUrl = @json($dashboardDataUrl ?? route('admin.live-dashboard.data'));
        const tvMode = @json($tvMode ?? false);
        ...
        function syncFullscreenButtonState() {
            ...
            document.body.classList.toggle('dashboard-fullscreen', Boolean(document.fullscreenElement));
            fullscreenToggleBtn.textContent = document.fullscreenElement ? 'Exit Fullscreen' : 'Fullscreen';
        }
        ...
        async function toggleFullscreenMode() {
            try {
                ...
                // Ignore fullscreen rejection to keep dashboard functional.
            } finally {
        ...
        if (tvMode) {
            ...
            // Try opening in fullscreen for TV mode.
            requestFullscreenMode().finally(syncFullscreenButtonState);
        }
        ...
        function shouldHideDailyScoreByQuery() {
            const params = new URLSearchParams(window.location.search);
            return params.has('month');
        }

        function applyDailyScoreVisibility() {
            const shouldHide = shouldHideDailyScoreByQuery();
            document.getElementById('daily-score-head')?.classList.toggle('hidden', shouldHide);
            document.querySelectorAll('[data-daily-score-cell]').forEach((el) => {
        ...
        function renderEmployeeRows(employees = []) {
            ...
            if (!tbody) return;
            const hideDailyScoreForCurrentView = shouldHideDailyScoreByQuery();

            if (!Array.isArray(employees) || employees.length === 0) {
                tbody.innerHTML = '<tr><td colspan="7" class="px-6 py-10 text-center text-gray-300 font-semibold">No employee performance data available for selected filter.</td></tr>';
                initTablePagination();
                return;
            }
            ...
            tbody.innerHTML = employees.map((emp) => {
                ...
                return `<tr class="table-row employee-row">
                    ...
                    <td class="px-4 py-2 text-center ${hideDailyScoreForCurrentView ? 'hidden' : ''}" data-daily-score-cell>
        ...
        async function refreshDashboardData() {
            try {
                const requestUrl = tvMode
                    ? `${dashboardDataUrl}${window.location.search || ''}`
                    : `${dashboardDataUrl}${window.location.search || ''}`;
        ...
        function syncLeaderAvatar(name, photoUrl = '') {
            ...
            if (img) {
                img.style.display = '';
                img.alt = n || 'Leader';
                img.src = photoUrl || leaderAvatarFallbackUrl(n);
            }
        }
        ...
        syncLeaderAvatar(document.getElementById('leader-name')?.textContent || 'Leader', leaderPhoto || '');
        applyDailyScoreVisibility();
        ...
        // Realtime dashboard refresh on score/lead updates.
        (function initLiveDashboardPusher() {
