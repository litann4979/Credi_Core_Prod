...
<head>
    ...
    <style>
        * {
        ...
        .employee-selection .search-box {
    position: relative;
    width: 100%;
}

.employee-selection .search-input {
    width: 100%;
    padding: 8px 12px 8px 36px;
    border: 1px solid #d1d5db;
    border-radius: 8px;
...
.employee-selection .search-icon {
    position: absolute;
    left: 12px;
    top: 50%;
    transform: translateY(-50%);
...
<body>
    ...
    <div class="main-content">
        @include('admin.Components.header', ['title' => 'Task Management', 'subtitle' => 'Create and manage team tasks'])
                    ...
                    <form id="taskForm" onsubmit="submitTask(event)">
                        ...
                        <div class="employee-selection">
                            ...
                            <div class="selection-toggle">
                                ...
                                <button type="button" class="toggle-btn" id="teamleadBtn" onclick="toggleSelection('individual_teamlead')">Individual Team Leads</button>
                                <button type="button" class="toggle-btn" id="operationBtn" onclick="toggleSelection('individual_operation')">Individual Operations</button>
                                <button type="button" class="toggle-btn" id="allBtn" onclick="toggleSelection('all')">All Members</button>
                            </div>

                            <!-- Operations multi-select section -->
                            <div class="selection-section" id="operationListSection" style="display: none;">
    <label class="form-label">Select Operations</label>
    <div class="search-box" style="margin-bottom: 12px;">
        <input type="text" class="search-input operation-search" placeholder="Search operations..." oninput="filterList('operationList', this.value)">
        <i class="fas fa-search search-icon"></i>
    </div>
    <div id="operationList" class="employee-list"></div>
</div>
    ...
    <div class="search-box" style="margin-bottom: 12px;">
        <input type="text" class="search-input employee-search" placeholder="Search employees..." oninput="filterList('employeeList', this.value)">
        <i class="fas fa-search search-icon"></i>
    </div>
    <div id="employeeList" class="employee-list"></div>
</div>

                            <!-- Teamlead multi-select -->
                           <div class="selection-section" id="teamleadListSection" style="display: none;">
    <label class="form-label">Select Team Leads</label>
    <div class="search-box" style="margin-bottom: 12px;">
        <input type="text" class="search-input teamlead-search" placeholder="Search team leads..." oninput="filterList('teamleadList', this.value)">
        <i class="fas fa-search search-icon"></i>
    </div>
    <div id="teamleadList" class="employee-list"></div>
</div>
    ...
    <div id="viewTaskModal" class="modal-overlay" style="display: none;">
        <div class="modal-content" style="width: 500px; background: white; padding: 24px; border-radius: 8px;">
            ...
            <button onclick="closeViewModal()" style="margin-top: 20px;" class="btn-secondary">Close</button>
        </div>
    </div>
    ...
    <script>
        ...
        const operations = @json($operations); // Now it's global
    ...
    items.forEach(item => {
        ...
        const roleElement = item.querySelector('.employee-role');
    ...
    employees.forEach(employee => {
        ...
        employeeItem.style.display = 'flex'; // Ensure default display
    ...
    teamleads.forEach(teamlead => {
        ...
        item.style.display = 'flex'; // Ensure default display
    ...
    operations.forEach(operation => {
        ...
        item.className = 'employee-item';
        item.style.display = 'flex'; // Ensure default display
        item.onclick = () => toggleOperation(operation.id);

        item.innerHTML = `
            <div class="employee-checkbox" id="checkbox-op-${operation.id}"></div>
            <div class="employee-avatar">${operation.avatar || ''}</div>
            <div class="employee-info">
                <div class="employee-name">${operation.name}</div>
                <div class="employee-role">Operation</div>
            </div>
        `;
        ...
        function toggleOperation(operationId) {
            const checkbox = document.getElementById(`checkbox-op-${operationId}`);
            const index = selectedOperations.indexOf(operationId);
        ...
        function toggleSelection(mode) {
            ...
            document.getElementById('operationBtn').classList.toggle('active', mode === 'individual_operation');
            ...
            document.getElementById('operationListSection').style.display = mode === 'individual_operation' ? 'block' : 'none';
            document.getElementById('allTeamMessage').style.display = mode === 'all' ? 'block' : 'none';

            // Update hidden input for target_type
            document.getElementById('targetType').value = mode;

            // Clear search inputs when switching modes
    document.querySelector('.employee-search').value = '';
    document.querySelector('.teamlead-search').value = '';
    document.querySelector('.operation-search').value = '';

    // Reset filtering
    filterList('employeeList', '');
    filterList('teamleadList', '');
    filterList('operationList', '');


            // Reset selections for other sections
            if (mode !== 'individual') {
                selectedEmployees = [];
            ...
            if (mode !== 'individual_operation') {
                selectedOperations = [];
                document.querySelectorAll('#operationList .employee-checkbox').forEach(checkbox => {
                    checkbox.classList.remove('checked');
                });
            }
        ...
        function submitTask(event) {
            ...
            } else if (assignmentMode === 'individual_operation') {
                selectedOperations.forEach(id => formData.append('operations[]', id));
            }
        ...
        function validateForm() {
            ...
            if (assignmentMode === 'individual_teamlead' && selectedTeamleads.length === 0) {
                showNotification('Please select at least one team lead', 'error');
                return false;
            }
            if (assignmentMode === 'individual_operation' && selectedOperations.length === 0) {
                showNotification('Please select at least one operation', 'error');
                return false;
            }
            ...
            paginatedTasks.forEach(task => {
                ...
                taskCard.innerHTML = `
                    ...
                    <div class="task-actions" style="margin-top: 10px; display: flex; gap: 8px;">
                        <button class="btn-primary" onclick="event.stopPropagation(); editTask(${task.id})">Edit</button>
                        <button class="btn-secondary" onclick="event.stopPropagation(); viewTask(${task.id})">View</button>
                    </div>
                `;
    ...
    document.getElementById('dueDate').value = formatDateForInput(task.due_date);
            ...
            if (assignmentMode === 'individual') {
                ...
                employees.forEach(employee => {
                    ...
                    if (checkbox) checkbox.classList.toggle('checked', selectedEmployees.includes(employee.id));
                });
            } else if (assignmentMode === 'individual_teamlead') {
                selectedTeamleads = task.assignees.map(a => a.id);
                teamleads.forEach(teamlead => {
                    const checkbox = document.getElementById(`checkbox-tl-${teamlead.id}`);
                    if (checkbox) checkbox.classList.toggle('checked', selectedTeamleads.includes(teamlead.id));
                });
            } else if (assignmentMode === 'individual_operation') {
                selectedOperations = task.assignees.map(a => a.id);
                operations.forEach(operation => {
                    const checkbox = document.getElementById(`checkbox-op-${operation.id}`);
                    if (checkbox) checkbox.classList.toggle('checked', selectedOperations.includes(operation.id));
                });
