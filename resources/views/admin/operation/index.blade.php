...
<head>
    ...
    <style>
        /* --- MODERN UI THEME (Consistent with Employee/Leads Pages) --- */
        :root {
            --primary-50: #eff6ff;
            --primary-100: #dbeafe;
            --primary-500: #3b82f6;
...
<body>
    ...
    <div class="main-container">
        ...
        <div class="page-hero mt-2">
            <div>
                <h1 class="page-title">
                    <i class="fas fa-users-cog me-2"></i>Operation Management
                </h1>
                <p class="page-subtitle">Manage your operations team members efficiently.</p>
            </div>
                ...
                <div class="stat-card-modern active-card">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            <p class="stat-label">Active Operations</p>
                        </div>
                                ...
                                <td class="text-end">
                                    <div class="d-inline-flex gap-2">
                                        <button class="btn btn-warning btn-sm text-white shadow-sm" style="border-radius: 8px;"
                                                onclick="editOperation({{ $teamlead->id }}, '{{ $teamlead->name }}', '{{ $teamlead->email }}', '{{ $teamlead->phone }}', '{{ $teamlead->address ?? '' }}', '{{ $teamlead->profile_photo ? asset('storage/' . ltrim($teamlead->profile_photo, 'storage/')) : '' }}')"
                                                title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </button>

                                        @if($teamlead->trashed())
                                            <form action="{{ route('admin.operations.activate', $teamlead->id) }}" method="POST" style="display:inline;">
                                                @csrf
                                        ...
                                        @else
                                            <form action="{{ route('admin.operations.deactivate', $teamlead->id) }}" method="POST" style="display:inline;">
                                                @csrf
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-5">
                                    <div class="d-flex flex-column align-items-center justify-content-center text-gray-400">
                                        ...
                                        <p class="mb-0 fw-medium">No operations found.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title d-flex align-items-center gap-2" id="operationModalLabel">
                        <i class="fas fa-user-plus"></i> Add Operation
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body p-5">
                    <form id="operationForm" action="{{ route('admin.operations.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
    ...
    <script>
        function resetForm() {
            ...
            document.getElementById('operationForm').action = "{{ route('admin.operations.store') }}";
            ...
            document.getElementById('submitButton').innerHTML = '<i class="fas fa-save me-2"></i>Add Operation';
            document.getElementById('operationModalLabel').innerHTML = '<i class="fas fa-user-plus me-2"></i>Add Operation';
        }

        function editOperation(id, name, email, phone, address, photo) {
            document.getElementById('operationForm').action = "{{ url('admin/operations') }}/" + id;
            ...
            document.getElementById('submitButton').innerHTML = '<i class="fas fa-save me-2"></i>Update Operation';
            document.getElementById('operationModalLabel').innerHTML = '<i class="fas fa-user-edit me-2"></i>Update Operation';
        ...
        document.getElementById('operationSearchInput')?.addEventListener('input', function () {
            const query = this.value.toLowerCase().trim();
            const rows = document.querySelectorAll('#operationTableBody tr');
