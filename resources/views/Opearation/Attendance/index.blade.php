...
<head>
    ...
    <title>Operations Attendance - Lead Management System</title>
...
<body>
    ...
    <div class="main-content">
         @include('Opearation.Components.header', ['title' => 'Attendance Management', 'subtitle' => 'Give your Attendance here'])
                    ...
                    <div class="current-info">
                        <div class="info-grid">
                            ...
                            <div class="info-item">
                                ...
                                <span>{{ auth()->user()->name ?? 'Operations User' }}</span>
                            </div>
                            <div class="info-item">
                                <label>Action</label>
                    ...
                    <div class="detail-left">
                        <div class="employee-info">
                            ...
                            <div class="employee-details">
                                <h3 id="modalEmployeeName">{{ auth()->user()->name ?? 'Operations User' }}</h3>
                                <p id="modalEmployeeRole">Operations Manager</p>
...
function checkTodayStatus() {
    ...
    try {
        const response = await fetch(`/operations/attendance/${id}`, {
            headers: {
        ...
        document.getElementById('modalEmployeePhoto').src = record.checkin_image || `https://ui-avatars.com/api/?name={{ auth()->user()->name ?? 'Operations User' }}&background=10b981&color=fff&size=120`;
        ...
        if (checkInTime) {
            ...
            let diffInMinutes = Math.floor((effectiveCheckout - checkInTime) / 60000);
            if (diffInMinutes > 60) {
                diffInMinutes -= 60; // subtract 1-hour break
            }
            const hours = Math.floor(diffInMinutes / 60);
...
async function submitAttendance() {
    ...
    try {
        const url = isCheckedIn ? '/operations/attendance/check-out' : '/operations/attendance/check-in';
        ...
        if (response.ok) {
            ...
            if (isCheckedIn) {
                ...
                if (existingIndex !== -1) {
                    attendanceData[existingIndex] = { ...attendanceData[existingIndex], ...result.attendance };
                }
            } else {
                // Add new record for check-in
                attendanceData.unshift(result.attendance);
            }
            filteredAttendance = [...attendanceData]; // Sync filtered data
            checkTodayStatus(); // Update UI with new status
            renderAttendance(); // Re-render attendance cards
            updateStats();
            updateResultsCount();
...
function openComplaintModal() {
        ...
        async function submitComplaint(event) {
            ...
            try {
                const response = await fetch('/operations/attendance/complaint', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Content-Type': 'application/json',
