...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Main Content Grid - Modified for larger table */
    .leads-content-grid {
        display: block;
    }
    /* Lead Form - Hidden by default, can be shown if needed */
    .lead-form-container {
        background: white;
        border-radius: 12px;
        padding: 24px;
    ...
    /* Leads Table Container - Made larger */
    ...
    /* Capitalize text in the All Leads table */
    .leads-table th,
    .leads-table td {
        text-transform: uppercase;
    }
    /* Capitalize text in the Lead Details modal */
    .lead-detail-grid,
    ...
    /* Ensure numeric and date fields are not affected unnecessarily */
    .lead-detail-grid input[type="number"],
    .lead-detail-grid input[type="date"] {
        text-transform: none;
    }
    .lead-type-display {
        font-size: 15px;
...
<body>
    ...
    <div class="main-content">
        @include('Opearation.Components.header', ['title' => 'Leads Management', 'subtitle' => 'Create, view, and manage your leads'])
        <div class="dashboard-container">
            <!-- Advanced Filters Section -->
            <div class="page-header">
                <div class="header-content">
                    <h1 class="page-title">Leads Management</h1>
                    <p class="page-subtitle">Filter and manage your leads efficiently</p>
                </div>
                ...
                <div class="lead-detail-left">
                    ...
                    <div class="lead-basic-info">
                        ...
                        <div class="lead-contact">
                            ...
                            <div class="contact-item"><i class="fas fa-university"></i><span id="modalLeadBankName"></span></div>
                        </div>
                    </div>
                </div>
...
<div class="detail-item">
    ...
    <select id="modalLeadCityDropdown" class="editable-field" style="display: none;">
        <option value="">Select City</option>
                            ...
                            <div class="detail-item"><label>Loan Account Number</label><input type="text" id="modalLeadAccountNumber" class="editable-field" disabled></div>
    ...
    <select id="modalLeadType" class="editable-field" style="display: none;" disabled>
        ...
        <option value="home_loan">Home Loan</option>
                            ...
                            <div class="detail-item"><label>Salary</label><input type="number" id="modalsalary" class="editable-field" disabled></div>
                            <div class="detail-item"><label>Bank Name</label><input type="text" id="modalBank" class="editable-field" disabled></div>
            ...
            <div class="modal-content">
                <div class="confirm-message">
                    ...
                    <div class="form-group full-width" style="margin-top: 20px;">
                        <label for="voiceRecording" style="font-weight: 600; margin-bottom: 12px; display: block;">
                            <i class="fas fa-microphone"></i> Voice Recording (Optional)
                        </label>
                        ...
                        <div style="display: flex; flex-direction: column; gap: 12px;">
                            ...
                            <div style="display: flex; gap: 8px; align-items: center;">
                                <button type="button" id="startRecordBtn" class="btn-primary" style="flex: 1; padding: 10px;" onclick="startRecording()">
                                    <i class="fas fa-microphone"></i> Start Recording
                                </button>
                                <button type="button" id="stopRecordBtn" class="btn-danger" style="flex: 1; padding: 10px; display: none;" onclick="stopRecording()">
                                    <i class="fas fa-stop"></i> Stop Recording
                                </button>
                            </div>

                            <!-- Recording Status -->
                            <div id="recordingStatus" style="display: none; padding: 12px; background: #fee2e2; border-radius: 8px; text-align: center; color: #991b1b; font-weight: 600;">
                                <i class="fas fa-circle" style="animation: pulse 1.5s infinite;"></i> Recording... <span id="recordingTimer">0:00</span>
                            </div>

                            <!-- Audio Preview -->
                            <div id="audioPreviewContainer" style="display: none; padding: 12px; background: #f0fdf4; border-radius: 8px;">
                                <label style="font-size: 13px; color: #166534; font-weight: 600; margin-bottom: 8px; display: block;">
                                    <i class="fas fa-check-circle"></i> Recording Complete - Preview:
                                </label>
                                <audio id="audioPreview" controls style="width: 100%; margin-top: 8px;"></audio>
                                <button type="button" class="btn-secondary" style="width: 100%; margin-top: 8px; padding: 8px;" onclick="clearRecording()">
                                    <i class="fas fa-trash"></i> Clear & Re-record
                                </button>
                            </div>
                        </div>
                    </div>
  ...
  function numberToWords(number) {
    ...
    if (filters.classList.contains('active')) {
        ...
        function populateFilterOptions() {
            const states = [...new Set(allLeads.map(lead => lead.state).filter(Boolean))].sort();
            const districts = [...new Set(allLeads.map(lead => lead.district).filter(Boolean))].sort();
            const cities = [...new Set(allLeads.map(lead => lead.city).filter(Boolean))].sort();
            const leadTypes = [...new Set(allLeads.map(lead => lead.leadType).filter(Boolean))].sort();

            populateSelect('stateFilter', states);
            populateSelect('districtFilter', districts);
            populateSelect('cityFilter', cities);
            populateSelect('leadTypeFilter', leadTypes);
        }
    ...
    document.getElementById('modalLeadInitials').textContent = lead.name ? lead.name.charAt(0).toUpperCase() : '';
                    ...
                    document.getElementById("modalLeadBankName").textContent = lead.bank_name ?? 'N/A';
                   ...
                   const leadTypeDisplay = lead.lead_type ? strReplaceUnderscore(lead.lead_type) : 'N/A';
...
document.getElementById('modalLeadType').value = lead.lead_type || '';
                    ...
                    document.getElementById("modalsalary").value = lead.salary ? `${lead.salary}` : '';
                    document.getElementById("modalBank").value = lead.bank_name ? `${lead.bank_name}` : '';

                    const voiceElement = document.getElementById("modalLeadVoiceRecording");
                    if (lead.voice_recording) {
                        // Get file extension to determine MIME type
                        const filePath = lead.voice_recording;
                        ...
                        const audioUrl = `/storage/${filePath}`;
                                ...
                                <div class="document-item">
                                    ...
                                    ${uploaded ? `
                                        ...
                                        <form method="POST" action="/operations/leads/${leadId}/documents/${doc.document_id}/delete" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this document?');">
                                            ...
                                            <button type="submit" style="margin-left: 10px; color: red;" ${isApprovedOrDisbursed ? 'disabled="disabled"' : ''}>Delete</button>
                                        </form>
                                    ` : `
                                        ${isApprovedOrDisbursed ? `
                                            <p style="color: #6b7280;">Upload disabled (Lead is ${lead.status})</p>
                                        ` : `
        ...
        window.showLoginModal = function(leadId) {
            window.currentLeadId = leadId;
    ...
    updateLeadStatus(window.currentLeadId, 'login', null, loanAccountNumber);
        ...
        // Modal functions for approve, reject, disburse
        window.showApproveModal = function(leadId) {
            window.currentLeadId = leadId;
    const loanAccountNumber = document.getElementById('modalLeadAccountNumber').value;
    const message = loanAccountNumber ?
        "This lead already has a loan account number. You can update it below if needed:" :
    ...
    document.getElementById('approveModal').classList.add('active');
        ...
        window.showRejectModal = function(leadId) {
            if (!leadId) {
        ...
        window.showDisburseModal = function(leadId) {
           window.currentLeadId = leadId;
    const loanAccountNumber = document.getElementById('modalLeadAccountNumber').value;
    const message = loanAccountNumber ?
        "This lead already has a loan account number. You can update it below if needed:" :
    ...
    updateLeadStatus(window.currentLeadId, 'approved', null, loanAccountNumber);
            ...
            updateLeadStatus(window.currentLeadId, 'rejected', reason, null, voiceRecording);
            closeModal('rejectModal');
        };

      window.confirmDisburse = function() {
 ...
 window.updateLeadStatus = function (leadId, status, reason = '', loanAccountNumber = '', voiceRecording = null) {
    if (!leadId) {
        console.error('No leadId provided');
        showNotification('Please select a lead first.', 'error');
        return;
    ...
    // If voice recording is present, use FormData; otherwise use JSON
    if (voiceRecording) {
        const formData = new FormData();
        formData.append('status', status);
        if (loanAccountNumber) {
            formData.append('loan_account_number', loanAccountNumber);
        }
        if (reason) {
            formData.append('reason', reason);
        }
        formData.append('voice_recording', voiceRecording);
...
window.confirmDelete = function (leadId) {
            if (confirm('Are you sure you want to delete this lead? This action cannot be undone.')) {
                fetch(`/operations/leads/${leadId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                    }
        ...
        function strReplaceUnderscore(str) {
            return str ? str.replace(/_/g, ' ').replace(/\b\w/g, char => char.toUpperCase()) : 'N/A';
        }
...
async function loadCities(districtId) {
    ...
    try {
        ...
        city: document.getElementById('modalLeadCityDropdown').options[document.getElementById('modalLeadCityDropdown').selectedIndex].text,
                ...
                bank_name: document.getElementById('modalBank').value,
            };
    ...
    .catch(error => {
        ...
        window.startRecording = async function() {
            try {
                ...
                showNotification('Failed to access microphone. Please allow microphone permissions.', 'error');
            }
        };
        ...
        window.clearRecording = function() {
            ...
            showNotification('Recording cleared. You can record again.', 'info');
        };
        ...
        window.confirmReject = function() {
            ...
            // Convert recorded blob to file if exists
            let voiceFile = null;
            if (recordedBlob) {
                const timestamp = new Date().getTime();
                voiceFile = new File([recordedBlob], `rejection_voice_${timestamp}.webm`, { type: 'audio/webm' });
            }

            updateLeadStatus(window.currentLeadId, 'rejected', reason, null, voiceFile);
            closeModal('rejectModal');
        ...
        // Updated updateLeadStatus Function
        window.updateLeadStatus = function (leadId, status, reason = '', loanAccountNumber = '', voiceRecording = null) {
            if (!leadId) {
                console.error('No leadId provided');
                showNotification('Please select a lead first.', 'error');
            ...
            if (voiceRecording) {
                ...
                .then(data => {
                    showNotification(data.message, 'success');
                    if (data.voice_recording_path) {
                        console.log('Voice recording saved at:', data.voice_recording_path);
                    }
                    closeModal('leadDetailModal');
                    closeModal(`${status}Modal`);
                    refreshLeads();
