...
<body>
        ...
        <div class="page-wrapper">
            ...
            <div class="content-card">
                <div class="card-header-custom">
                    <h5 class="card-title"><i class="fas fa-list text-warning"></i> Operations List</h5>
                ...
                <div class="table-responsive">
                    <table class="table-custom">
                        ...
                        <tbody>
                            @forelse($teamleads as $op)
                                <tr>
                                    <td>
                                        <div class="user-cell">
                                            @if($op->profile_photo)
                                                <img src="{{ asset('storage/' . ltrim($op->profile_photo,'storage/')) }}" alt="" class="user-avatar">
                                            @else
                                                <div class="user-avatar-placeholder">{{ strtoupper(substr($op->name, 0, 1)) }}</div>
                                            @endif
                                            <div class="user-info">
                                                <h6>{{ $op->name }}</h6>
                                                <span>{{ $op->email }}</span>
                                    ...
                                    <td>
                                        <div class="d-flex flex-column text-muted small" style="font-size: 0.75rem;">
                                            <span><i class="fas fa-phone me-1"></i> {{ $op->phone }}</span>
                                            @if($op->dob)
                                                <span><i class="fas fa-birthday-cake me-1"></i> {{ \Carbon\Carbon::parse($op->dob)->format('d M') }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td><span class="text-muted small">{{ Str::limit($op->address ?? 'N/A', 30) }}</span></td>
                                    <td>
                                        @if($op->trashed())
                                    ...
                                    <td class="text-end">
                                        <div class="action-buttons-container">
                                            <button class="action-btn btn-view" onclick="viewOperation({{ $op->id }})" title="View">
                                                <i class="fas fa-eye"></i>
                                            </button>
                                            
                                            <button class="action-btn btn-salary" onclick="openSalaryModal({{ $op->id }})" title="Salary">
                                                <i class="fas fa-money-bill-wave"></i>
                                            </button>

                                            <button class="action-btn btn-edit" 
                                                onclick="editOperation(
                                                    {{ $op->id }}, '{{ addslashes($op->name) }}', '{{ $op->email }}', '{{ $op->phone }}', '{{ addslashes($op->address ?? '') }}', '{{ $op->profile_photo ? asset('storage/' . ltrim($op->profile_photo, 'storage/')) : '' }}', '{{ $op->dob ? \Carbon\Carbon::parse($op->dob)->format('Y-m-d') : '' }}'
                                                )" title="Edit">
                                                <i class="fas fa-pencil-alt"></i>
                                            </button>

                                            @if($op->trashed())
                                                <form action="{{ route('hr.operations.activate', $op->id) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="action-btn btn-status-on" title="Activate"><i class="fas fa-toggle-off"></i></button>
                                                </form>
                                            @else
                                                <form action="{{ route('hr.operations.deactivate', $op->id) }}" method="POST">
                                                    @csrf
                            ...
                            @empty
                                <tr><td colspan="5" class="text-center py-4 text-muted small">No Operations found.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="operationModalLabel">Add Operation</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="operationForm" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" id="method_field" name="_method" value="POST">
                        <input type="hidden" id="operation_id" name="operation_id">
                        <input type="hidden" name="designation" value="operations">
                        <div class="row g-3">
                ...
                <div class="modal-body p-4">
                    ...
                    <div id="viewModalContent" style="display:none;">
                        <div class="text-center mb-3">
                            ...
                            <span class="badge badge-custom badge-role">Operation</span>
                        </div>
    ...
    <script>
        ...
        const routes = {
            store: "{{ route('hr.operations.store') }}",
            updateBase: "{{ url('hr/operations') }}",
        };

        // --- Operation Add/Edit Logic ---
        function openAddModal() {
            const form = document.getElementById('operationForm');
            form.reset();
            form.action = routes.store;
            document.getElementById('method_field').value = 'POST';
            document.getElementById('operationModalLabel').innerHTML = '<i class="fas fa-user-plus me-2"></i>Add Operation';
            document.getElementById('submitButton').innerText = 'Save Operation';
        ...
        function editOperation(id, name, email, phone, address, photo, dob) {
            const form = document.getElementById('operationForm');
            form.action = `${routes.updateBase}/${id}/update`;
            document.getElementById('method_field').value = 'POST';
            ...
            document.getElementById('operationModalLabel').innerHTML = '<i class="fas fa-user-edit me-2"></i>Edit Operation';
            document.getElementById('submitButton').innerText = 'Update Operation';
        ...
        function viewOperation(id) {
            ...
            fetch(`${routes.updateBase}/${id}/details`)
                ...
                .then(data => {
                    const op = data.data;
                    document.getElementById('view_name').innerText = op.name;
                    document.getElementById('view_email').innerText = op.email;
                    document.getElementById('view_phone').innerText = op.phone;
                    document.getElementById('view_dob').innerText = data.formatted_dob;
                    document.getElementById('view_avatar').src = data.profile_url || `https://ui-avatars.com/api/?name=${op.name}&background=f97316&color=fff`;
        ...
        function openSalaryModal(userId) {
            ...
            fetch(`/hr/operations/${userId}/salary`)
                .then(res => res.json())
                .then(res => {
                    if(res.success) {
                        document.getElementById('salary_emp_name').innerText = res.user_name;
                        document.getElementById('salary_emp_role').innerText = res.designation || 'Operation';
        ...
        document.getElementById('salaryForm').addEventListener('submit', function(e) {
            ...
            fetch(`/hr/operations/${uid}/salary`, {
                method: 'POST',
