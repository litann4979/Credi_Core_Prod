...
class AdminLeaveApprovalController extends Controller
{
    public function create()
    {
        ...
        return view('admin.leave_approval.index', compact('leaveRequests'));
    }
    ...
    public function update(Request $request, Leave $leave)
{
    ...
    if ($leave->applied_to !== Auth::id()) {
        return redirect()->route('admin.leave.approvals')->with('error', 'Unauthorized action.');
    }
    ...
    if ($validated['action'] === 'approved' && $leave->leave_type !== 'paid_leave') {
        ...
        if (!$balance || $balance->balance < $leave->total_days) {
            return redirect()->route('admin.leave.approvals')->with('error', 'Insufficient leave balance.');
        }
    }
...
NotificationHelper::sendGeneralNotification(
    ...
    "Your leave request has been {$validated['action']}.",
    null
);
    ...
    return redirect()->route('admin.leave.approvals')->with('success', $message);
}
}
